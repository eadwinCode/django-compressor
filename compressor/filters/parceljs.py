    ...
    def input(self, **kwargs):
        ...
        try:
            ...
            if self.outfile is not None:
                self.outfile.close()
        return ('js',smart_text(filtered)), ('css', smart_text(css_filtered)) if css_filtered else ('css', css_filtered) 
    ...
    def input(self, **kwargs):
        ...
        if _kind == 'file':
            if (settings.COMPRESS_ENABLED):
                self.command = f"{self.command} {kwargs.get('filename')}" +" --no-source-maps --no-cache -d /tmp/ --out-file {outfile}"
            else:
                self.command = f"{self.command} {kwargs.get('filename')}" + " --no-minify --no-source-maps  -d /tmp/ --out-file {outfile}"
        else:
            if (settings.COMPRESS_ENABLED):
                self.command = f"{self.command}" + " {infile} --no-source-maps --no-cache -d /tmp/ --out-file {outfile}"
            else:
                self.command = f"{self.command}" + " {infile} --no-minify --no-source-maps  -d /tmp/ --out-file {outfile}"
